/*
    ...
    The original file 'AURecogniseORG.cpp' can be found in this directory.

    Main modifications:
        Call Python functions of Emotion Classifier (AUDdisplay.py) on each frame and provide it with detected action units. 
...
void AURecogniser::recogniseFromVideoCapture(cv::VideoCapture& cap)
{
    ...
    latencyLogFile.open("/home/janciovec/Documents/jFiles/Skola/Cambridge/II/_Project/_src/DisplayingModule/LatencyEvaluation/auStartTimes.dat");

    //Import Python script
    PyObject *pName, *pModule, *pFunc, *pFunc2, *pFunc3, *pArgs, *pArgs2;
    Py_Initialize();
    pName = PyString_FromString("AUDdisplay");                      // name of python file
    //pName = PyString_FromString("AUDdisplayExperiment");          // name of python file FOR EVALUATION OF HUMAN EXPERIMENT
    ...
    pArgs = PyTuple_New(1);
    pFunc2 = PyObject_GetAttrString(pModule, "updateEmotion");      // For each frame, send detected action units
    ...
    PyTuple_SetItem(pArgs, 0, PyString_FromString(ipPort));
    ...
    while (!interruptSignal)
    {
        ...
        /////JO///// Stop if no new frames!
        if (frame.rows == 0 || frame.cols == 0)
            break;
        // original did not seem to work
        //if (!frame.cols) {
        //     break;
        // }
        //////////
...
//            std::cout     << rawDevPitch << '\t' << rawDevYaw << std::endl;
        ...
        /////JO/////
        // Call Python function to display emotion, send binary sequence of detected action units (as a string)
        char aus[config.AUlist.size()];
        for (size_t i=0; i<config.AUlist.size(); ++i)
            aus[i] = AUdecisions[config.AUlist[i]] ? '1' : '0';
        PyTuple_SetItem(pArgs2, 0, PyString_FromString(aus));
        ...
        if (config.visualiseOutput)
            cv::imshow("frame", frame);
        /////JO/////int keyClicked = (char) cv::waitKey(10);
        /////JO/////if (keyClicked == 32)
        int keyClicked = (char) cv::waitKey(5);
        if (keyClicked == 27)
            break;
        }
